...
fn main() {
    ...
    let s = ModInt::new((0..n).into_iter().map(|i| a[i]).sum());
    let mut sub = ModInt::zero();
    let mut subs = ModInt::zero();
    let mut ans = s.clone();
    for i in 1..n {
        sub += ModInt::new(a[i - 1] + a[n - i]);
        subs += sub;
        ans += (s * ModInt::new(i + 1) - subs) * ModInt::new(i + 1).inv();
    }
    println!("{}", ans.val);
}
