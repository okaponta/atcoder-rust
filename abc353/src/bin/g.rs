...
fn main() {
    ...
    let mut seg_pre = SegmentTree::new(n, -1 << 60, |a, b| a.max(b));
    let mut seg_suf = SegmentTree::new(n, -1 << 60, |a, b| a.max(b));
    seg_pre.update(0, 0);
    seg_suf.update(0, 0);
    let mut ans = 0;
    for (t, p) in tp {
        let ti = t as i64;
        let tmp = p + (seg_pre.query(0..t + 1) - c * ti).max(seg_suf.query(t..n) + c * ti);
        ans = ans.max(tmp);
        seg_pre.update(t, tmp + c * ti);
        seg_suf.update(t, tmp - c * ti);
    }
    println!("{}", ans);
}
...
impl<T: Copy + Debug, F> fmt::Debug for SegmentTree<T, F>
where
    F: Fn(T, T) -> T,
{
