...
fn main() {
    let comb = Binom::init(100, MOD);
    input! {t:usize}
    for _ in 0..t {
        input! {mut n:usize, k:usize}
        let mut ones = n.count_ones() as usize;
        let mut ans = ModInt::zero();
        if ones == k {
            ans = ans + ModInt::new(n);
        }
        for i in 0..61 {
            if n >> i & 1 == 0 {
                continue;
            ...
            let c = ModInt::new(comb.get(i, k - ones));
            ans = ans + c * ModInt::new(n);
            ans =
                ans + ModInt::new((1 << i) - 1) * c * ModInt::new(k - ones) * ModInt::new(i).inv();
        }
        println!("{}", ans.val);
    }
}
