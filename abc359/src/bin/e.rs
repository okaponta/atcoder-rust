...
fn main() {
    ...
    let mut b = vec![0; n];
    for i in (0..n).rev() {
        while let Some(Reverse((hj, j))) = heap.pop() {
            if hj <= h[i] {
                b[j] = j - i;
            } else {
                heap.push(Reverse((hj, j)));
                break;
            }
        }
        heap.push(Reverse((h[i], i)));
    }
    ...
    let mut ans = vec![];
    for i in 0..n {
        let bef = seg_tree.get_range(i + 1 - b[i], i).i;
        let exp = h[i] * b[i];
        tmp += exp - bef;
        seg_tree.range_update(i + 1 - b[i], i, Param::new(h[i]));
        ans.push(tmp);
    }
...
// params 更新に使うパラメータ
// 更新結果のノードiの値を返却する
fn update(_p: Node, len: usize, params: Param) -> Node {
    Node { i: params.p * len }
}
