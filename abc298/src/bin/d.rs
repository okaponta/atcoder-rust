...
fn main() {
    ...
    qu.push_back(1);
    let mut mint = ModInt::one();
    let mut ten = vec![ModInt::one()];
    for i in 0..600005 {
        ten.push(ten[i] * ModInt::new(10));
    }
    for _ in 0..q {
        input! {qi: u8}
        if qi == 1 {
            input! {x: usize}
            qu.push_back(x);
            mint = mint * ModInt::new(10) + ModInt::new(x);
        } else if qi == 2 {
            let del = qu.pop_front().unwrap();
            mint -= ModInt::new(del) * ten[qu.len()];
        } else {
            println!("{}", mint.val)
        }
    }
