...
fn main() {
    ...
    let mut seg = SegmentTree::new(w + 1, -1 << 60, |a, b| a.max(b));
    seg.update(0, 0);
    for (l, r, v) in lrv {
        for i in (l..=w).rev() {
            let min = if i < r { 0 } else { i - r };
            let mx = seg.query(min..i + 1 - l);
            seg.update_tmp(i, seg.get(i).max(mx + v));
        }
        seg.update_all();
    }
    println!("{}", seg.get(w).max(-1));
}
