...
fn main() {
    ...
    for i in 0..d {
        for &j in &ap[i] {
            seg_tree.range_update((j + 1).saturating_sub(w), j, Param::new(1));
        }
    }
    let mut ans = seg_tree.get_range(0, 200_001);
    for i in d..200_001 {
        for &j in &ap[i] {
            seg_tree.range_update((j + 1).saturating_sub(w), j, Param::new(1));
        }
        ...
        ans = ans.max(seg_tree.get_range(0, 200_001));
    }
    println!("{}", ans.i);
}

// let mut seg_tree = LazySegTree::new(values, merge, update, composition);
