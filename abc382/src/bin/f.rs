...
fn main() {
    input! {
        ...
        w:usize,
        n:usize,
        rcl:[(Usize1,Usize1,Usize1);n],
    }
    let ord = (0..n)
        .into_iter()
        .sorted_by_key(|&i| rcl[i].0)
        .rev()
        .collect::<Vec<_>>();
    //println!("{:?}", ord);
    let mut ans = vec![0; n];
    let mut seg_tree = LazySegTree::new(vec![Node::new(); w], merge, update, composition);
    for i in ord {
        let (_r, c, l) = rcl[i];
        let bef = seg_tree.get_range(c, c + l).i;
        seg_tree.range_update(c, c + l, Param::new(bef + 1));
        ans[i] = h - bef;
    }
    for ans in ans {
        println!("{}", ans);
...
// params 更新に使うパラメータ
// 更新結果のノードiの値を返却する
fn update(_p: Node, _len: usize, params: Param) -> Node {
    Node { i: params.p }
}
