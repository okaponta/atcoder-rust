...
fn main() {
    ...
    let val = (0..n)
        .into_iter()
        .map(|i| Node::new(a[i], b[i]))
        .collect::<Vec<_>>();
    let mut seg = LazySegTree::new(val, merge, update, composition);
    for _ in 0..q {
        input! {qi:u8, l:Usize1, r: Usize1}
        if qi == 1 {
            input! {x:usize}
            seg.range_update(l, r, Param::new(x, 0));
        } else if qi == 2 {
            input! {x:usize}
            seg.range_update(l, r, Param::new(0, x));
        } else {
            println!("{}", seg.get_range(l, r).ci);
        }
    }
}
...
impl Node {
    pub fn new(ai: usize, bi: usize) -> Self {
        Self {
            ai: ai % MOD,
            bi: bi % MOD,
            ci: (ai * bi) % MOD,
...
impl Param {
    ...
    fn add(&mut self, other: Param) -> Self {
        self.pa = (self.pa + other.pa) % MOD;
        self.pb = (self.pb + other.pb) % MOD;
        *self
    }
}
...
fn merge(p1: Node, p2: Node) -> Node {
    let a = (p1.ai + p2.ai) % MOD;
    let b = (p1.bi + p2.bi) % MOD;
    let c = (p1.ci + p2.ci) % MOD;
    Node {
        ai: a,
        bi: b,
        ci: c,
...
// params 更新に使うパラメータ
// 更新結果のノードiの値を返却する
fn update(p: Node, len: usize, params: Param) -> Node {
    let ai = (p.ai + params.pa * len) % MOD;
    let bi = (p.bi + params.pb * len) % MOD;
    let mut ci = p.ci;
    ci += params.pa * p.bi;
    ci %= MOD;
    ci += params.pb * p.ai;
    ci %= MOD;
    ci += ((params.pa * params.pb) % MOD) * len;
    ci %= MOD;
    Node { ai, bi, ci }
}
