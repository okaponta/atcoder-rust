...
fn solve(n: usize, a: Vec<usize>) {
    let map = compress(&a);
    let m = map.len();
    let mut seg1 = SegmentTree::new(m, 0, max);
    let mut c1 = vec![0; n];
    for i in 0..n {
        c1[i] = seg1.query(0..map[&a[i]]) + 1;
        seg1.update(map[&a[i]], c1[i]);
    }
    let mut seg2 = SegmentTree::new(m, 0, max);
    let mut c2 = vec![0; n];
    for i in (0..n).rev() {
        c2[i] = seg2.query(map[&a[i]] + 1..m) + 1;
        seg2.update(map[&a[i]], c2[i]);
    }
    let max = (0..n).into_iter().map(|i| c1[i] + c2[i]).max().unwrap();
    let ans = (0..n)
        .into_iter()
        .filter(|&i| c1[i] + c2[i] == max)
        .map(|i| i + 1)
        .collect::<Vec<_>>();
    println!("{}", ans.len());
