...
fn main() {
    ...
    let mut seg = SegmentTree::new(n, (0, 1 << 60), |a, b| if a.1 < b.1 { a } else { b });
    for i in 0..n {
        let mut val = 0;
        for &j in &edges[i] {
            val += a[j];
        }
        seg.update_tmp(i, (i, val));
    }
    seg.update_all();
    let mut used = vec![false; n];
    let mut ans = 0;
    for _ in 0..n {
        let (k, v) = seg.query(0..n);
        used[k] = true;
        ans = ans.max(v);
        for &i in &edges[k] {
            if !used[i] {
                let bef = seg.query(i..i + 1);
                seg.update(i, (i, bef.1 - a[k]));
            }
        }
        seg.update(k, (k, 1 << 60));
    }
