...
fn main() {
    ...
    let mut ans = (n - 2) * (n - 2);
    let mut col = SegmentTree::new(2 * n, 0, |a, b| a.max(b));
    let mut row = SegmentTree::new(n - 1, 0, |a, b| a.max(b));
    col.update(n - 2, n - 1);
    row.update(n - 2, n - 1);
    for (i, x) in x {
        if i == 0 {
            let pos = col.max_right(0, n, |e| x <= e) - 1;
            let bef = row.query(x - 1..x);
            ans -= pos - bef;
            row.update(x - 1, pos);
        } else {
            let pos = row.max_right(0, n, |e| x <= e) - 1;
            let bef = col.query(x - 1..x);
            ans -= pos - bef;
            col.update(x - 1, pos);
        }
    }
