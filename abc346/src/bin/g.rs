...
fn main() {
    ...
    let mut res = 0;
    let mut seg = LazySegTree::new(vec![Node::new(); n + 1], merge, update, composition);
    for i in 1..=n {
        for &((l, r), x) in &rect[i] {
            seg.range_update(l, r - 1, Param::new(x));
        }
        let node = seg.get_range(i, n);
        if node.i == 0 {
            res += (n + 1 - i) as i64 - node.j;
        } else {
            res += (n + 1 - i) as i64;
        }
    }
...
fn merge(p1: Node, p2: Node) -> Node {
    if p1.i == p2.i {
        ...
        return Node { i: p1.i, j: p1.j };
    } else {
        return Node { i: p2.i, j: p2.j };
    }
}
