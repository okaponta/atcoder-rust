...
fn main() {
    ...
    let mut seg = SegmentTree::new(n, (0, 0), |a, b| mx(a, b));
    for i in 0..m {
        seg.update(a[i], (a[i], seg.query(a[i]..a[i] + 1).1 + 1));
        println!("{}", seg.query(0..n).0 + 1);
    }
}

fn mx(a: (usize, usize), b: (usize, usize)) -> (usize, usize) {
    if a.1 < b.1 {
        return b;
    }
    a
